...
void computeAccelerations() 
{
    int i, j,izero,ium,idois,jzero,jum,jdois;
    double f, rSqd,aux1,diff0,diff1,diff2,term1, term2, Pot,aux2,di0_f,di1_f,di2_f;
    ...
    for (i = 0; i < N-1; i++) 
    {
        ...
        for (j = i+1; j < N; j++) 
        {
            ...
            diff0=r[izero] - r[jzero];
            diff1=r[ium] - r[jum];
            diff2=r[idois] - r[jdois];

            rSqd=diff0*diff0+diff1*diff1+diff2*diff2;
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (i=0; i<N; i++) 
    {
        ...
        ium=izero+1;
        idois=ium+1;
        r[izero] += v[izero]*dt + 0.5*a[izero]*dt*dt;
        r[ium] += v[ium]*dt + 0.5*a[ium]*dt*dt;
        r[idois] += v[idois]*dt + 0.5*a[idois]*dt*dt;


        v[izero] += 0.5*a[izero]*dt;
        v[ium] += 0.5*a[ium]*dt;
        v[idois] += 0.5*a[idois]*dt;
...
void initializeVelocities() 
{
    ...
    for (i=0; i<N; i++) {
        vSqdSum += v[i*3]*v[i*3];
        vSqdSum += v[i*3+1]*v[i*3+1];
        vSqdSum += v[i*3+2]*v[i*3+2];
    }
